...
async function createTestVideo(supabase) {
  ...
  const testStoryboard = {
    scenes: [
      {
        scene_number: 1,
        description: 'Hook scene showing someone feeling energized',
        image_prompt: 'A person feeling energized and happy after exercise',
        text: 'Want to feel amazing every day?',
        duration: 3
      },
      {
        scene_number: 2,
        description: 'Body scene showing exercise benefits',
        image_prompt: 'Exercise boosts your energy, mood, and health',
        text: 'Exercise boosts your energy, mood, and health.',
        duration: 4
      },
      {
        scene_number: 3,
        description: 'CTA scene encouraging action',
        image_prompt: 'Start moving today for a better tomorrow',
        text: 'Start moving today for a better tomorrow!',
        duration: 3
      }
    ]
  };
...
async function runTest() {
  try {
    ...
    const videoId = await createTestVideo(supabase);
      ...
      // Setup buckets first
  info('Setting up Supabase storage buckets...');
  const { execSync } = require('child_process');
  const path = require('path');
  
  try {
    const setupScriptPath = path.join(__dirname, 'scripts', 'setup-buckets.cjs');
    execSync(`node "${setupScriptPath}"`, { 
      stdio: 'inherit',
      cwd: process.cwd()
    });
    success('Bucket setup completed');
  } catch (setupError) {
    warning('Bucket setup failed, but continuing with test');
  }
